...
class KbRepository:
    ...
    @staticmethod
    async def create(name: str, description: str = "") -> dict:
        ...
        logger.info("KB created: id=%s, name=%s", kb_id, name)
        return {
            "id": kb_id,
            "name": name,
    ...
    async def update_doc_count(kb_id: str, delta: int = 1) -> None:
        ...
        db = await get_db()
        await db.execute(
            "UPDATE knowledge_bases SET doc_count = doc_count + ? WHERE id = ?",
            (delta, kb_id),
        )
        await db.commit()
    ...
    async def update_conv_count(kb_id: str) -> None:
        ...
        await db.execute(
            "UPDATE knowledge_bases SET conv_count = ? WHERE id = ?",
            (count, kb_id),
        )
        await db.commit()
